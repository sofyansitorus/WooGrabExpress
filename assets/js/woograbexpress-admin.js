(function($) {
	...
	var wooGrabExpress = {
		_inputLatId: "woocommerce_woograbexpress_origin_lat",
		_inputLngId: "woocommerce_woograbexpress_origin_lng",
		_mapWrapperId: "woograbexpress-map-wrapper",
		_mapSearchId: "woograbexpress-map-search",
		_mapCanvasId: "woograbexpress-map-canvas",
		_zoomLevel: 16,
		_keyStr:
				...
				setTimeout(function() {
					...
					if (!isMethodAdded) {
						...
						$("select[name='add_method_id']")
							.val(woograbexpress_params.method_id)
							.trigger("change");
					}
				}, 200);
			}
			...
			try {
				...
				$.getScript(
					"https://maps.googleapis.com/maps/api/js?key=" +
						...
						woograbexpress_params.language,
					function() {
						self._buildGoogleMaps();
					}
				);
		...
		_buildGoogleMaps: function() {
			...
			var marker = new google.maps.Marker({
				...
				position: curLatLng,
				draggable: true,
				icon: woograbexpress_params.marker
			});

			var infowindow = new google.maps.InfoWindow({ maxWidth: 350 });

			if (curLat == defaultLat && curLng == defaultLng) {
				infowindow.setContent(woograbexpress_params.txt.drag_marker);
