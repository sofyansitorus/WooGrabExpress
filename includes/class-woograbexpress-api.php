	...
	public function calculate_distance( $args = array(), $is_validation = false ) {
		try {
			...
			if ( empty( $response_body ) ) {
				throw new Exception( __( 'API response is empty', 'woograbexpress' ) );
			}
			...
			if ( $json_last_error_msg && 'No error' !== $json_last_error_msg ) {
				...
				throw new Exception( sprintf( __( 'Error occured while decoding API response: %s', 'woograbexpress' ), $json_last_error_msg ) );
			}
			...
			if ( ! empty( $errors ) ) {
				...
				foreach ( $errors as $error_key ) {
					if ( isset( $error_template[ $error_key ] ) ) {
						throw new Exception( __( 'API Response Error', 'woograbexpress' ) . ': ' . $error_template[ $error_key ] );
					}
				}
			}

			throw new Exception( __( 'API Response Error', 'woograbexpress' ) . ': ' . __( 'No results found', 'woograbexpress' ) );
		} catch ( Exception $e ) {
