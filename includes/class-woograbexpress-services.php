...
class WooGrabExpress_Services {
	...
	public function get_fields() {
		$raw_fields = array(
			...
			'per_km_min_distance' => array(
				'title'             => __( 'Per Kilometer Cost Minimum Distance (km)', 'woograbexpress' ),
				'type'              => 'number',
		...
		foreach ( $this->get_data() as $sevice_key => $sevice ) {
			...
			foreach ( $raw_fields as $key => $field ) {
				if ( 'title' === $key ) {
					$field['default'] = sprintf( 'GrabExpress - %s', $sevice['label'] );
	...
	public function calculate_cost( $service_id, $distance, $envelope, $settings ) {
		...
		if ( ! array_key_exists( $service_id, $data ) ) {
			...
			return new WP_Error( 'invalid_service_id', sprintf( __( 'Invalid service ID: %1$s', 'woograbexpress' ), $service_id ) );
		}
		...
		if ( $max_distance && $distance > $max_distance ) {
			// translators: %1$s is service ID.
			return new WP_Error( 'max_distance_exceeded', sprintf( __( 'Shipping distance exceeded the maximum setting: %1$s', 'woograbexpress' ), $service_id ) );
		}
		...
		if ( ! $multiple_drivers ) {
			...
			if ( $envelope_weight > $max_weight ) {
				// translators: %1$s is service ID.
				return new WP_Error( 'max_weight_exceeded', sprintf( __( 'Package weight exceeded the maximum setting: %1$s', 'woograbexpress' ), $service_id ) );
			}

			// Validate package dimension.
			if ( $envelope_dimension > $max_dimension ) {
				// translators: %1$s is service ID.
				return new WP_Error( 'max_dimension_exceeded', sprintf( __( 'Package dimension exceeded the maximum setting: %1$s', 'woograbexpress' ), $service_id ) );
			}
		}
