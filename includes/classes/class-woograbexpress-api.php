	...
	public function calculate_distance( $args = array(), $is_validation = false ) {
		try {
			...
			if ( $json_last_error_msg && 'No error' !== $json_last_error_msg ) {
				...
				throw new Exception( sprintf( __( 'Error occurred while decoding API response: %s', 'woograbexpress' ), $json_last_error_msg ) );
			}
			...
			throw new Exception( __( 'No results found', 'woograbexpress' ) );
		} catch ( Exception $e ) {
			return new WP_Error( 'api_request', __( 'Google Distance Matrix API error', 'woograbexpress' ) . ': ' . $e->getMessage() );
		}
	}
}
