 ...
 */
class WooGrabExpress_Shipping_Method extends WC_Shipping_Method {
	...
	private function migrate_data() {
		...
		$data_version_option_key = 'woograbexpress_data_version_' . $this->get_instance_id();

		$data_version = get_option( $data_version_option_key );

		if ( $data_version && version_compare( WOOGRABEXPRESS_DATA_VERSION, $data_version, '<=' ) ) {
			return;
		}
		...
		foreach ( $migrations as $migration ) {
			...
			$this->show_debug( sprintf( __( 'Data migrated to version %s', 'woograbexpress' ), $data_version ) );
		}
	}
	...
	public function init_form_fields() {
		$form_fields = array(
			...
			'field_group_store_location'  => array(
				...
				'description' => sprintf( __( 'This plugin requires Google API Key and API services enabled for Distance Matrix API, Maps JavaScript API, Geocoding API, Places API. <a href="%s" target="_blank">Click here</a> to go to Google API Console to get API Key and to enable the service.', 'woograbexpress' ), 'https://cloud.google.com/maps-platform/#get-started' ),
			),
			'api_key'                     => array(
				'title'             => __( 'Distance Calculator API Key', 'woograbexpress' ),
				'type'              => 'woograbexpress',
				'orig_type'         => 'text',
				'desc_tip'          => __( 'API Key used to calculate the shipping address distance. Required Google API Service: Distance Matrix API.', 'woograbexpress' ),
				'description'       => __( 'Required API Services: <strong>Distance Matrix API</strong>. Please disable the API Key restriction unless you know what you are doing, then set it to <strong>IP addresses</strong>.', 'woograbexpress' ),
				'default'           => '',
				'is_required'       => true,
				'class'             => 'woograbexpress-api-key-input',
				'custom_attributes' => array(
					'data-link' => 'api_key',
					'data-key'  => 'api_key',
					'readonly'  => 'readonly',
			...
			'api_key_picker'              => array(
				'title'             => __( 'Location Picker API Key', 'woograbexpress' ),
				'type'              => 'woograbexpress',
				'orig_type'         => 'text',
				'desc_tip'          => __( 'API Key used to render the location picker map. Required Google API Services: Maps JavaScript API, Geocoding API, Places API.', 'woograbexpress' ),
				'description'       => __( 'Required API Services: <strong>Maps JavaScript API, Geocoding API, Places API</strong>. Please disable the API Key restriction unless you know what you are doing, then set it to <strong>HTTP referrers</strong>.', 'woograbexpress' ),
				'default'           => '',
				'is_required'       => true,
				'class'             => 'woograbexpress-api-key-input',
				'custom_attributes' => array(
			...
			'origin_type'                 => array(
				'title'             => __( 'Store Origin Data Type', 'woograbexpress' ),
				'type'              => 'woograbexpress',
				'orig_type'         => 'select',
			...
			'origin_lat'                  => array(
				...
				'default'           => '',
				'is_required'       => true,
				'class'             => 'woograbexpress-field--origin',
				'custom_attributes' => array(
			...
			'preferred_route'             => array(
				...
				'type'        => 'woograbexpress',
				'orig_type'   => 'select',
				'description' => __( 'Preferred route that will be used for calculation if API provide several routes', 'woograbexpress' ),
				'desc_tip'    => true,
			...
			'round_up_distance'           => array(
				...
				'type'        => 'woograbexpress',
				'orig_type'   => 'checkbox',
				'description' => __( 'Round up the calculated shipping distance with decimal to the nearest absolute number.', 'woograbexpress' ),
				'desc_tip'    => true,
			),
			'show_distance'               => array(
		...
		foreach ( $this->services as $service ) {
			$form_fields[ 'field_group_services_' . $service->get_slug() ] = array(
				...
				'title'       => sprintf( __( 'Service Settings: %s', 'woograbexpress' ), $service->get_label() ),
		...
		$form_fields['field_group_third_party'] = array(
			...
			'orig_type'   => 'title',
			'class'       => 'woograbexpress-field-group',
			'title'       => __( 'Third-Party Settings', 'woograbexpress' ),
			'description' => __( 'Settings added by third-party plugins.', 'woograbexpress' ),
		);
		 ...
		 *
		 *      add_filter( 'woograbexpress_form_fields', 'my_woograbexpress_form_fields', 10, 2 );
		 *
		 *      function my_woograbexpress_form_fields( $form_fields, $instance_id ) {
		 *          return array();
		 *      }
		 */
	...
	public function generate_js_template_html() {
		ob_start();
		?>
		<script type="text/template" id="tmpl-woograbexpress-button">
			<button href="{{data.href}}" class="button button-secondary woograbexpress-button {{data.class}}">{{{ data.text }}}</button>
		</script>
		...
		<script type="text/template" id="tmpl-woograbexpress-map-search-panel">
			<div id="woograbexpress-map-search-panel" class="woograbexpress-map-search-panel woograbexpress-hidden expanded">
				<button type="button" id="woograbexpress-map-search-panel-toggle" class="woograbexpress-map-search-panel-toggle woograbexpress-map-search-element"><span class="dashicons"></button>
	...
	public function generate_store_location_picker_html( $key, $data ) {
		...
		<tr valign="top" class="woograbexpress-row">
			<td colspan="2" class="woograbexpress-no-padding">
				<table id="woograbexpress-table-map-picker" class="form-table woograbexpress-table woograbexpress-table-map-picker" cellspacing="0">
					<tr valign="top">
						<td colspan="2" class="woograbexpress-no-padding">
	...
	public function validate_woograbexpress_field( $key, $value ) {
		...
		if ( $field ) {
			...
			try {
				...
				throw new Exception( sprintf( __( 'Error: %s', 'woograbexpress' ), $e->getMessage() ) );
			}
	...
	private function api_request( $args = array(), $cache = true ) {
		 ...
		 *
		 *      add_filter( 'woograbexpress_api_request_pre', 'my_woograbexpress_api_request_pre', 10, 4 );
		 *
		 *      function my_woograbexpress_api_request_pre( $false, $args, $cache, $obj ) {
		...
		try {
			 ...
			 * This example shows how you can modify the $pre var via custom function:
			 *
			 *      add_filter( 'woograbexpress_api_request', 'my_woograbexpress_api_request', 10, 2 );
			 *
			 *      function my_woograbexpress_api_request( $result, $obj ) {
			 *          // Return the response data array
			 *          return array(
			 *              'distance'          => 40,
			 *              'distance_text'     => '40 km',
	...
	public function calculate_shipping( $package = array() ) {
		try {
			...
			foreach ( $this->services as $service ) {
				...
				* @return array
				*/
				$shipping_cost = apply_filters( 'woograbexpress_calculate_shipping_cost_pre', false, $service->get_slug(), $api_response['distance'], $envelope, $this->instance_settings );
	...
	private function get_origin_info( $package ) {
		 ...
		 *
		 *      add_filter( 'woograbexpress_origin_info', 'my_woograbexpress_origin_info', 10, 3 );
		 *
		 *      function my_woograbexpress_origin_info( $origin_info, $package, $instance_id ) {
		 *          return array(
	...
	private function get_destination_info( $package ) {
		 ...
		 *
		 *      add_filter( 'woograbexpress_destination_info_pre', 'my_woograbexpress_destination_info_pre', 10, 3 );
		 *
		 *      function my_woograbexpress_destination_info_pre( $false, $package, $instance_id ) {
		...
		foreach ( $shipping_fields['data'] as $key => $field ) {
			...
			if ( woograbexpress_is_calc_shipping() && ! apply_filters( 'woocommerce_shipping_calculator_enable_' . $field_key, true ) ) { // phpcs:ignore WordPress.NamingConventions
				continue;
			}
			...
			try {
				...
				if ( $required && ! $value ) {
					...
					throw new Exception( sprintf( __( 'Shipping destination field is empty: %s', 'woograbexpress' ), $field_key ) );
				}
				...
				if ( $value && 'postcode' === $field_key && ! empty( $package['destination']['country'] ) ) {
					...
					if ( ! WC_Validation::is_postcode( $value, $country_code ) ) {
						// translators: %s is field key.
						throw new Exception( sprintf( __( 'Shipping destination field is invalid: %s', 'woograbexpress' ), $field_key ) );
					}
				}
		 ...
		 * This example shows how you can modify the shipping destination info via custom function:
		 *
		 *      add_filter( 'woograbexpress_destination_info', 'my_woograbexpress_destination_info', 10, 3 );
		 *
		 *      function my_woograbexpress_destination_info( $destination_info, $package, $instance_id ) {
		 *          return '1600 Amphitheater Parkway, Mountain View, CA, 94043, United State';
		 *      }
		 */
	 ...
	 *
	 * @param WooGrabExpress_Migration $a Object 1 migration handler.
	 * @param WooGrabExpress_Migration $b Object 1 migration handler.
	 * @return int
	 */
